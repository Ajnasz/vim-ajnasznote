...
local function get_tags()
  ...
  for _, tree in ipairs(parser:trees()) do
    for _0, qmatch, _1 in q:iter_matches(tree:root(), parser:source()) do
      if qmatch then
        for id, nodes in pairs(qmatch) do
          local name = q.captures[id]
          if (name == "tag") then
            for _2, node in ipairs(nodes) do
...
local function get_h1_callback(bufnr, cb)
  local parser = vim.treesitter.get_parser(bufnr, "markdown")
  local function _11_(err, trees)
    if err then
      return cb(err)
    else
      return cb(nil, vim.treesitter.get_node_text(get_h1_node(parser), parser:source()))
    end
  end
  return parser:parse({1, 50}, _11_)
end
return {get_h1 = get_h1, get_h1_cb = get_h1_callback, get_meta_title = get_meta_title, get_tags = get_tags, get_meta_dict = get_meta_dict, get_meta_end_line = get_meta_end_line}
